...
#this is my own code, it was written without consulting other students.
#Junyuan Ke 2148073
...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        #find most likely position of each remaining uncaptured ghost, choose action that minimizes dist to closest ghpst

        ghostPosition = []
        for ghostDistribution in livingGhostPositionDistributions:
            ghostPosition.append(ghostDistribution.argMax())

        #find most likely position of living ghost
        position = [0,0]
        maxProb = 0
        for i, pos in enumerate(ghostPosition):
            if livingGhostPositionDistributions[i][pos] > maxProb:
                maxProb = livingGhostPositionDistributions[i][pos]
                position = pos

        #find minimum distance to the ghost
        bestAtion = None
        minDist = float('inf')
        for action in legal:
            nextPosition = Actions.getSuccessor(pacmanPosition,action)
            thisDist = self.distancer.getDistance(nextPosition, position)
            if thisDist < minDist:
                minDist = thisDist
                bestAtion = action
        return bestAtion
