...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        #emmission Modesl: P(noisyDistance | TrueDistance)
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        #if a ghost is eatern, update all particles to jail
        if noisyDistance == None:
...
class JointParticleFilter:
    ...
    def observeState(self, gameState):
        ...
        ** Remember ** We store particles as tuples, but to edit a specific particle,
        ...
        for i in range(self.numGhosts):
            #if eaten, put in jain
            if noisyDistances[i] == None:
                allPossible = util.Counter()
                for p in belief:
                    allPossible[self.getParticleWithGhostInJail(p,i)] += belief[p]
                belief = allPossible

            else:
                for p in belief:
                    dist = util.manhattanDistance(p[i], pacmanPosition)
                    belief[p] = belief[p] * emissionModels[i][dist]
